...
--    -- 1024
  ...
  local discriminator_head =nn.Sequential()
    ...
    --:add(nn.Sigmoid())
    ---- 1
    discriminator_head:add(SpatialConvolution(ndf * 8, ngen+1, 4, 4))
    --discriminator_head:add(nn.Sigmoid())
    -- state size: (ngen+1) x 1 x 1
  ...
  local info_head = nn.Sequential()
    ...
    info_head:add(SpatialConvolution(ndf * 8, 1024, 4, 4))
    info_head:add(SpatialBatchNormalization(1024)):add(nn.LeakyReLU(0.2, true))
    info_head:add(nn.Reshape(1024))
    info_head:add(nn.Linear(1024,128))
...
--    -- 1 x 28 x 28
    ...
    for i=1,opt.ngen do
        ...
        G['generator'..i]:add(SpatialFullConvolution(ngf * 2, ngf, 4, 4, 2, 2, 1, 1))
        G['generator'..i]:add(SpatialBatchNormalization(ngf)):add(nn.ReLU(true))
        -- state size: (ngf) x 32 x 32
        G['generator'..i]:add(SpatialFullConvolution(ngf, nc, 4, 4, 2, 2, 1, 1))
